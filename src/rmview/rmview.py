...
class rMViewApp(QApplication):
    ...
    def __init__(self, config_files, override_orientation=None):
        ...
        self.trailPen = QPen(
            QColor(self.config.get("pen_color", "red")), max(1, self.pen_size // 3)
        )
        ...
        if "background_color" in self.config:
            self.viewer.setBackgroundBrush(
                QBrush(QColor(self.config.get("background_color")))
            )
    ...
    def ensureConnConfig(self):
        ...
        if type(address) is list:
            address, ok = QInputDialog.getItem(
                self.viewer, "Connection", "Address:", address
            )
            if ok and address:
        ...
        if (auth_method == "password" and "password" not in self.config["ssh"]) or (
            auth_method is None
            and "password" not in self.config["ssh"]
            and "key" not in self.config["ssh"]
        ):
            password, ok = QInputDialog.getText(
                self.viewer, "Configuration", "reMarkable password:", QLineEdit.Password
            )
            if ok:
        ...
        if (
            self.config["ssh"].get("insecure_auto_add_host")
            and "host_key_policy" not in self.config["ssh"]
        ):
            log.warning(
                "The 'insecure_auto_add_host' setting is deprecated, see documentation."
            )
        ...
        if log.isEnabledFor(logging.DEBUG):
            ...
            if "password" in self.config.get("ssh", {}):
                config_sanitized["ssh"]["password"] = "*" * len(
                    config_sanitized["ssh"]["password"]
                )
    ...
    def _checkConfigFilePermissions(self, file_path):
        ...
        if bool(st_mode & stat.S_IROTH) or bool(st_mode & stat.S_IWOTH):
            ...
            log.warn(
                'Config file "%s" is readable by others (permissions=%s). If your config '
                "file contains secrets (e.g. password) you are strongly encouraged to make sure "
                "it's not readable by other users (chmod 600 %s)"
                % (file_path, file_permissions, file_path)
            )
    ...
    def connected(self, ssh):
        ...
        if version not in [1, 2]:
            log.error(
                "Device is unsupported: '%s' [%s]",
                ssh.fullDeviceVersion,
                version or "unknown device",
            )
            QMessageBox.critical(
                None,
                "Unsupported device",
                "The detected device is '%s'.\nrmView currently only supports reMarkable 1 and 2."
                % ssh.fullDeviceVersion,
            )
            self.quit()
            return
        ...
        if backend == "auto":
            if ssh.softwareVersion >= (2, 9, 0, 0):
                ...
                if ssh.softwareVersion >= (2, 7, 0, 0):
                    log.warning(
                        "Detected version 2.7 or 2.8. The server might not work with these versions."
                    )
        ...
        self.pen = self.viewer.scene.addEllipse(
            0,
            0,
            self.pen_size,
            self.pen_size,
    ...
    def promptDependenciesInstall(self):
        mbox = QMessageBox(
            QMessageBox.NoIcon,
            "Missing components",
            "Your reMarkable is missing some needed components.",
        )
        ...
        if answer == 1:
            ...
            try:
                ...
                QMessageBox.critical(
                    None,
                    "Error",
                    "There has been an error while trying to install the required components on the tablet.\n%s\n."
                    % e,
                )
                self.quit()
    ...
    def movePen(self, x, y):
        ...
        if self.trail is not None:
            if self.trail is False:
                ...
                QTimer.singleShot(self.trailDelay // 2, lambda: t.setOpacity(0.5))
                QTimer.singleShot(
                    self.trailDelay, lambda: self.viewer.scene.removeItem(t)
                )
        self.pen.setRect(
            x - (self.pen_size // 2),
            y - (self.pen_size // 2),
...
def rmViewMain():
    ...
    parser.add_argument(
        ...
        nargs="?",
        help=(
            "An override path for the config files. If not set, the"
            " default '~/.config/rmview.json' is used"
        ),
    )
    ...
    loglevel_group.add_argument(
        "-q", "--quiet", action="store_true", help="Set log level to ERROR"
    )
    ...
    orientation_group.add_argument(
        ...
        "--landscape",
        action="store_true",
        help="Connect in 'landscape' orientation, overriding the configured layout",
    )
    orientation_group.add_argument(
        "-P",
        "--portrait",
        action="store_true",
        help="Connect in 'portrait' orientation, overriding the configured layout",
    )
    ...
    app = rMViewApp(
        config_files=args.config_files, override_orientation=override_orientation
    )
